...
module.exports = (function() { 'use strict';
  ...
  function upload(uploadUrl, parsedUri, uploadOptions, fileSize, fileName, callback, customOptions) {
    ...
    var options = getRequestOptions('POST', uploadUrl, headers, uploadOptions.uploadAgent, customOptions);
  ...
  * on the remote web server of if maxFileSize is provided, we can bail out if it's too large */
  ...
  function getRequestOptions(method, url, headers, agent, customOptions) {
    var options = {
      method: method,
      host: url.host,
      path: url.path,
  ...
  return {
    post: function(uri, options, customOptions, callback) {
      ...
      try {
        ...
        if (parsedUri.isValidUrl) {
          head(parsedUri, uploadOptions, 0, function(err, fileUrl, fileSize, fileName) {
            ...
            upload(uploadUrl, fileUrl, uploadOptions, fileSize, fileName, callback, customOptions);
          });
        }
        else {
          fs.exists(uri, function(exists) {
            ...
            fs.stat(uri, function(err, stats) {
              ...
              upload(uploadUrl, parsedUri, uploadOptions, stats.size, fileName, callback, customOptions);
            });
          });
        }
      }
